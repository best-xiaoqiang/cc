...
(function (global, factory) {
  ...
  function prepare () {
    ...
    self.createCtx = function () {
      var id = self.id;
      if (id) {
        self.ctx = uni.createCanvasContext(id);
      } else {
  ...
  function methods () {
    ...
    self.getCropperImage = function () {
      ...
      let toTempFilePathFn = function (params) {
        // #ifdef MP-ALIPAY
        console.log('self.ctx.toTempFilePath')
        return self.ctx.toTempFilePath(params)
        // #endif
        return uni.canvasToTempFilePath(params)
      }
      let successCb = function (res) {
        let path = res.tempFilePath
        console.log('res', res)
        // #ifdef MP-ALIPAY
        if (res.apFilePath) path = res.apFilePath
        // #endif
        isFunction(fn) && fn.call(self, path);
      }
